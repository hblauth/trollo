...
class Board extends React.Component {
    constructor(props) {
        ...
        this.state = {
            lists: [],
            // Is there a better way? [What is it] Can I stick all this in an 'editData' array?
    ...
    updateTodo(listIndex, todoIndex, newTodoText) {
        ...
        currentList.todos.splice(todoIndex, 1, { todoText: newTodoText, complete: false });
    ...
    checkAllComplete(listIndex) {
        ...
        todos.forEach((todo) => { // Use map instead? [performance] ramifications?
    ...
    toggleTodoComplete(listIndex, todoIndex) {
        const updatedLists = this.state.lists;
        updatedLists[listIndex].todos[todoIndex].complete = !updatedLists[listIndex].todos[todoIndex].complete;
        this.setState({
            lists: updatedLists,
        });
        this.checkAllComplete(listIndex);
    ...
    addListHandler(newListTitle) {
        this.setState({
            lists: [...this.state.lists, { title: newListTitle, todos: [], allComplete: false }],
        });
    }
    ...
    render() {
        ...
        return (
            <div>
                <div className="board">
                    {!listsList.length && 'Woot! You have nothing to do - go crack a cold one and put your feet up'}
                    {listsList}
                </div>
                <div className="controlDiv">
                    <div id="hackattack">Lists: {this.state.lists.length}   |   Todos:{(() => { let tdCount = 0; this.state.lists.map(l => tdCount += l.todos.length); return tdCount; })()}</div>
                    <Control
